  ...
  'audit-boundaries': {
    ...
    markdown: (r) => {
      const lines = [
        ...
        `- **Rules**: ${r.rulesApplied?.length || 0} (${r.isAutogenerated ? 'autogenerated' : 'configured'})`,
        ``,
      ];
      if (r.violations?.length) {
        lines.push(`| Source | Target | Type |`);
        lines.push(`| --- | --- | --- |`);
        lines.push(...r.violations.map((v) => `| ${v.sourceFile} | ${v.targetFile} | ${v.violationType} |`));
      }
      return lines.join('\n');
    },
    jsonl: (r) => {
      const rec = [];
      rec.push({ _type: 'summary', ok: r.ok, command: 'audit-boundaries', violationsCount: r.violationsCount, rulesAppliedCount: r.rulesApplied?.length || 0, isAutogenerated: r.isAutogenerated });
      pushRecord(rec, 'violation', r.violations);
      return rec.map(JSON.stringify).join('\n');
    },
  },
  'audit-smells': {
    ...
    markdown: (r) => {
      const lines = [
        `# Code Smells`,
        ``,
        `- **Findings**: ${r.smellsCount}`,
        ``,
      ];
      if (r.smells?.length) {
        lines.push(`| File | Function | Arms | Complexity |`);
        lines.push(`| --- | --- | --- | --- |`);
        lines.push(...r.smells.map((s) => `| ${s.file}:${s.lineStart || '?'} | ${s.functionName} | ${s.arms} | ${s.complexity} |`));
      }
      return lines.join('\n');
    },
    jsonl: (r) => {
      const rec = [];
      rec.push({ _type: 'summary', ok: r.ok, command: 'audit-smells', smellsCount: r.smellsCount });
      pushRecord(rec, 'smell', r.smells);
      return rec.map(JSON.stringify).join('\n');
    },
  },
  'query-stability': {
...
const AI_DIGEST = {
  ...
  'audit-boundaries': (r) => {
    const topRisks = [];
    const actions = [];
    const counts = { violations: r.violationsCount || 0 };
    if (r.violationsCount > 0) {
      topRisks.push({ category: 'boundaries', severity: 'high', count: r.violationsCount, message: `${r.violationsCount} architecture boundary violation(s)`, confidence: 0.9 });
      actions.push({ priority: 'P0', action: 'Review boundary violations and adjust rules or refactor imports' });
    }
    return { topRisks, actions, counts };
  },
  'audit-smells': (r) => {
    const topRisks = [];
    const actions = [];
    const counts = { smells: r.smellsCount || 0 };
    if (r.smellsCount > 0) {
      const severity = r.smellsCount > 5 ? 'high' : 'medium';
      topRisks.push({ category: 'smells', severity, count: r.smellsCount, message: `${r.smellsCount} code smell(s) detected (flat dispatcher / dominant branch)`, confidence: 0.85 });
      actions.push({ priority: 'P1', action: 'Refactor flat dispatchers into strategy/polymorphic dispatch patterns' });
    }
    return { topRisks, actions, counts };
  },
};
