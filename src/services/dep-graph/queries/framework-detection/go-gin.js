/**
 * Gin framework detection query — tree-sitter query for content-based detection.
 * Matches: gin.Context / gin.Default() / gin.New()
...
function postProcess(matches) {
  ...
  for (const match of matches) {
    ...
    if (pkg === 'gin' && (field === 'Context' || field === 'Default' || field === 'New')) {
      return {
        framework: 'gin',
        reason: 'gin-handler',
        isEntry: true,
