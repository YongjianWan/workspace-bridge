...
|---|---|---|---|---|---|---|
| **1** | **P0** | Formatters | `stats --format markdown` prints `[object Object]` for nested metrics data. | `node cli.js stats --cwd . --quiet`<br>**Target**: `src/cli/formatters/human-formatters.js` | Broken human readability. | Recursively stringify objects in `human-formatters.js`. |
| **2** | **P0** | CLI / Args | `--format json` is ineffective on L1 core commands. | `node cli.js audit-file --file src/services/container.js --format json`<br>**Target**: `src/cli/commands/audit-file.js` (and diff/summary) | Downstream JSON parser crash on Markdown. | Match `--format json` to the `--json` global flag. |
| **3** | **P0** | Config | Silent failure & fallback on corrupted `.workspace-bridge.json`. | `echo "invalid_json" > .workspace-bridge.json && node cli.js audit-summary --json`<br>**Target**: `src/services/file-index.js` | AI works with wrong file scope (escapes reference/). | Throw a hard parsing error and exit 1 on invalid JSON. |
| **4** | **P1** | Heuristics | Empty file (0 bytes) gets `severity: high` and 34 mention tests. | `touch empty.js && node cli.js audit-file --file empty.js --json`<br>**Target**: `src/tools/affected-tests.js` | Severe test execution overhead. | Skip stem/mention matching for empty or export-free files. |
| **5** | **P1** | CLI / Args | Invalid options (format, direction, mode, depth) are silently ignored. | `node cli.js audit-summary --format invalid --quiet` (returns exit 0)<br>**Target**: `src/cli/parse-args.js` | Deceptive exit 0 success. | Enforce strict parameter validation in `parse-args.js` (exit 2). |
| **6** | **P1** | Indexer | `--exclude` glob patterns (`*.test.js`, `src/**`) do not work. | `node cli.js audit-summary --exclude "*.test.js"` (includes tests)<br>**Target**: `src/services/file-index.js` | Test files pollute impact graphs. | Implement simple glob matching inside `file-index.js`. |
...
| **14** | **P1** | Indexer | Empty directories cause `audit-summary` to time out (60s). | `mkdir -p /tmp/empty && node cli.js audit-summary --cwd /tmp/empty`<br>**Target**: `src/services/file-index.js` | Failures on empty repos. | Return early in `file-index.js` if file count is 0. |
| **15** | **P1** | CLI / Args | Parameter bounds (e.g. `--max-depth 0`) print 50+ lines of help. | `node cli.js audit-summary --max-depth 0`<br>**Target**: `src/cli/parse-args.js` | Stream pollution. | Print single-line error messages instead of full help text. |
| **16** | **P1** | Parser | ESM syntax (`export const`) in CommonJS code crashes parsing engine. | Inject `export const dummy = 1;` into `src/services/container.js` and execute any command<br>**Target**: `src/services/parser.js` | Complete CLI crash. | Wrap parsing blocks in try-catch and output a graceful fallback error. |
| **17** | **P1** | Lifecycle | `init` in non-git directory followed by `audit-summary` triggers unhandled timeouts. | Run `init` and `audit-summary` in a raw folder without git initialization<br>**Target**: `src/services/file-index.js` | Silent blocking in non-git environments. | Gracefully exit when git status calls fail inside indexers. |
| **18** | **P1** | Config | Valid `.workspace-bridge.json` configuration ignoring the `archive` directory role. | Set `directoryRoles: {"reference": "archive"}` in `.workspace-bridge.json` and run: `node cli.js audit-summary --json --quiet`<br>**Target**: `src/services/file-index.js` | Excluded/archived folders are parsed anyway (`directoryRoles.reference=1845`), bloating index graphs. | Ensure the indexer strictly filters out folders matched under the `archive` role. |
| **19** | **P2** | Formatters | Markdown format lacks `validationAdvice` for L1 commands. | `node cli.js audit-file --file src/services/container.js --format markdown`<br>**Target**: `src/cli/formatters/human-formatters.js` | Missed recommended action context. | Implement Markdown templates for validation advice. |
| **20** | **P2** | CLI / Docs | `--fail-on-findings` flag is hidden from `--help`. | `node cli.js --help --all` (flag is missing)<br>**Target**: `src/cli/parse-args.js` | Undiscovered feature. | Expose option in CLI parameter documentation. |
| **21** | **P2** | Indexer | Orphan count fluctuates (0 vs 2) across runs. | Delete SQLite cache and run `node cli.js audit-map` twice<br>**Target**: `src/services/file-index.js` | Data reliability concerns. | Stabilize indexing cycles / cache invalidation. |
...
| **23** | **P2** | Security | `audit-security` JSON outputs `ruleId` but Markdown expects `rule`. | `node cli.js audit-security --json`<br>**Target**: `src/tools/security-tools.js` | Formatting naming divergence. | Synchronize output field naming conventions. |
| **24** | **P2** | Impact | `symbolImpact` misses certain symbols in complex destructuring imports. | `node cli.js audit-file --file src/services/container.js --json` (STATES missing)<br>**Target**: `src/services/resolvers.js` | False sense of safety. | Expand resolver mappings for multiple imports. |
...
| **26** | **P2** | Orchestrator| `--token-budget` downgrade occurs silently without metadata warning. | `node cli.js audit-summary --token-budget 50 --depth full --json`<br>**Target**: `src/tools/audit-assembler.js` | Undetected loss of fidelity. | Inject a `downgraded: true` flag in the metadata payload. |
