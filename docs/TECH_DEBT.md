...
| 集成测试（`spawn`/`runCli`）  | ~27   | ~21% | 已补充 `cli-integration-test.js` + `cli-integration-query-test.js` |
...
> 注：分类有重叠（如 `cache-concurrency-test.js` 既是集成测试也是并发测试），占比基于总文件数 134 独立计算，不互斥。
...
2. ✅ 集成/端到端测试分布已补全：针对 Stage 3.5 新增的 `query-hotspots`, `query-knowledge-risk`, `query-stability` 补充了全面的 E2E/集成测试，验证了各种格式化器与参数过滤逻辑，使测试类型分布更加均衡。
