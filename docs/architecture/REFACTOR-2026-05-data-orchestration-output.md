...
> 日期：2026-05-20  
> 状态：Wave 1（低垂果实）已完成；O1-O3 已完成；**U1 已完成**；Wave 2/3 待实施
...
（D4）**：`updateFiles()` finally 块中 `await cache.save()`，增量及时持久化。
...
（O5）**：`setTimeout` 回调中 `.catch()` 捕获异常，进程不崩溃。
...
1. `cli.js`：`determineExitCode()` ~~25 行 switch-case~~ → **已压至 4 行** O(1) 契约（`ok` + `hasFindings` + `regression.ok`），U2 核心目标已达成。剩余：13 个命令未返回 `hasFindings`（非阻塞）。
...
Layer 3: 输出层（可以独立做，与数据层无强依赖）
    ...
    ├── U1（Formatter 注册表）——消灭 switch ✅ 已完成
