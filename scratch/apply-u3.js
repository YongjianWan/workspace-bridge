...
const startIdx = lines.findIndex(l => l.startsWith('async function buildProjectOverview('));
const endIdx = lines.findIndex((l, i) => i > startIdx && l === '  };') + 1; // Includes '  };' and '}'
...
async function buildProjectOverview(args, container) {
  ...
  return {
    ...
    hotspots: rawData.hotspots.slice(0, 100),
    architectureAdvice: {
      cycleRefactorSuggestions: rawData.cycleRefactorSuggestions,
      couplingSplitSuggestions: rawData.mainlineFiles.length < 200 ? [] : rawData.couplingSplitSuggestions,
    },
    ...
    stability: rawData.stability.slice(0, 100),
    stabilityMeta: {
      totalCount: rawData.stability.length,
      truncated: rawData.stability.length > 100,
      limit: 100,
    },
...
// Add the new exported functions
const exportsIdx = lines.findIndex(l => l === 'module.exports = {');
lines.splice(exportsIdx + 1, 0, '  assembleOverviewData,\n  writeOverviewOutputs,');

fs.writeFileSync(targetPath, lines.join('\n'), 'utf8');
console.log('Successfully refactored overview-tools.js');
