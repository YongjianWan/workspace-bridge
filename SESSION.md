...
### 会话上下文
- 起点是"看项目做初步了解"，一路推到 L2-21：TECH_DEBT 把它记成「泛化 vs 平行」的架构选择，实读代码发现该选择**不存在**——`tier:'tier3'`（`builder.js:732`）、`confidence:0.3`（`:734`）、`resolutionMethod:'java-same-package'`（`:733`）三行相邻、扛的是同一个事实，判别器本来就语言中立，字符串那份是第三个副本。这半结论由读代码得出，不依赖测试。
- 全程与另一条 session 并行改同一个仓，实验数据被废两次（详见下）。

### 本轮完成
1. **架构判断**：L2-21 的「泛化 vs 平行」障碍不成立。Go 同包边只要生成时带 `tier='tier3'/confidence=0.3`，读侧（hasImplicit 族、L1-3 死导出、impact 归因）不需要知道 Go 存在。写侧照 Java 同形展开 + 删掉 `tryGoModule` 绑字母序首文件那段。
2. **证据核查（本轮主要产出，纯登记未改代码）**：对另一路写进 TECH_DEBT 的变异实验结论做独立复核，揪出两个洞——② 那跑（"冗余"结论的唯一承重）落在污染窗口内无可信复跑；④ 的逐处归因超出合并变异能给的。两条均已在 TECH_DEBT 转"待验"。
3. **依赖准确性缺口排序**（用户目标：文件/函数级依赖更准）：Go 绑首文件 > Python 标准库 23/290 > JVM manifest 未读。第三条实测 grep 坐实：`src/` 里 `pom.xml`/`build.gradle` **零次出现**，而 `resolvers/base.js` 已读 go.mod / package.json 四类字段 / Cargo.toml / requirements.txt+pyproject.toml 五种 manifest。
4. **留档一个反复被问的判断**：标准库/官方包**本体不引入图中**——影响分析从变更文件流向依赖方，标准库既非变更源亦非依赖方，永不出现在任何查询答案里。需要的是官方 **manifest**，不是官方**包**。

### 踩到的坑（下一轮省时间）
- **并行 session 会在跑中途 revert 你的变异**。全量要 6–9 分钟，这窗口足够对方还原源文件；结果是前半段测变异、后半段测干净代码，runner 照样给你一个漂亮的 266/266。最刺眼的症状是**超集变异比子集失败更少**（逻辑上不可能）。我先归因"并发 runner 抢 cache"，**错了**——查进程表 23 个 node 零个在跑测试，全是 MCP server。真因是文件被还原。**变异实验必须跑前跑后各验一次 diff。**
- **`node test/runner.js | tail -40` 会吞掉退出码**，拿到的是 `tail` 的 0。前三跑的 exit code 全是假的，只能读输出里的计数。后来改成不接管道 + `RUNNER_EXIT=$?`。
- **"孤儿 runner 进程"是我猜的，猜错了**。23 个 node 全是 MCP server（playwright/context7/server-github/kimi-*）。真按原方案"清干净 node 进程树"执行，会打断 MCP 工具链，零收益。**杀进程前先看 CommandLine。**
- `WB_TEST_CACHE_DIR` 是**逐测试读**的，不是 runner 全局开关——设了也只是部分隔离。

### 下一步（按依赖顺序）
1. **补一次干净的 ② 跑**（只杀 5 处字符串析取半 + 2 处换 `tier==='tier3'`），跑前跑后各验 diff、隔离 cache、不接管道。绿了「冗余」才算有背书；红了整个结论翻盘。
2. **四处各自单跑**，才有资格说哪处零覆盖、哪处被哪条测试守着。
3. 1、2 都清了再动 Go 写侧：包导入展开到该包全部非测试 `.go`、同包互引走 tier3、删 `tryGoModule` 绑首文件、CACHE_VERSION bump、cobra 复测。
4. **先跟另一路对齐谁收这个尾**——两条 session 同时改同一个仓、同时写同一份文档，今天已废掉两组实验数据，这是当前最该先解决的问题。

---

## 上一轮会话 (2026-07-28，架构讲解 → 九语言等价性实测)

### 会话上下文
- 起点：用户要求"看文档评估项目现状"，随后逐步深入——质疑外部评估的可信度 → 问"边是什么" → 要架构讲解 → 问"九种语言是否实际不均衡"。全程用实测替代文档复述。
- 期间仓库被另一路推进（`1ec45c0` / `117a179` L2-12 清零 / `3f3a452` / `03a4a2d`），本轮所有数字均在**当前 HEAD 上重测**过。

### 本轮完成
1. **`shared.js` 转手再导出的 `path` 删除**（`439d964`）：外部依赖闸挡住的 212 条假边，根因是一行 shorthand。变异验证——临时让 `_isExternalJsPackage` 恒返回 false，改前 1230 边/212 条 symbol-table（specifier 全是 `path`、target 全是 `shared.js`，无第二种形状），改后 1018 边/0 条。顺带修 `resolver-precision.js` 被 git 判为二进制（源码里嵌了两个真 NUL 字节当 Map 键分隔符，改 `\u0000` 转义）。全量 258/258。
2. **外部评估的证据核对**：一份外部给出的 L2-10 评估，机制与结论均属实（逐行核过 `shared.js:1`/`:195`、链尾挂载、末段提取、真值 3），但"`path` 60 条 + `debug`/`config`/`glob`/`semver` 碰撞合计 209"是**编的**——实测 212 条全部单 specifier 单 target，那几个名字来自代码注释里的假想例子。已据此在 TECH_DEBT 主动削弱 L2-10 自身论据（根因清除后，假边计数不再能证明"该策略在 JS 上必然产垃圾"，判决应靠"零正产出"）。
3. **九语言等价性实测**（本轮主要产出，纯登记未改代码）：十个最小 fixture 逐语言量边 + 闸隔离探针 + 注册表六钩子消费审计。发现 **C/C++ 恒 0 边**（新 L1-4）、L2-11 缺口从 2 个语言订正到 **4 个**、`unresolved` 语义错位（新 L2-13）、`isBuiltIn` 死配置（新 L3-6）、Vue/Svelte 正则抽符号（新 L3-7）。详见 CHANGELOG 同日条目。

### 验证状态
- 十个 fixture 在当前 HEAD 重跑一致；C/C++ 零边 5 次重跑全部复现。
...
- **核对外部评估要区分「机制对」和「数据对」**。机制描述可以完全正确，而支撑数字是把代码注释里的举例当实测复述。判断依据必须回到能重跑的探针。
- **探针要能隔离被测对象**。第一版闸探针把"闸拦住"和"符号表恰好没这个名字"混为一谈，结论全错；改成让符号表对任何名字都命中，才测的是闸。

---

## 上轮会话 (2026-07-27 → 07-28)
...
11. **reference/ 编制满编**（13 仓 / 9 语言）：六个旧仓全部 pull 到最新（GitNexus +495 / code-review-graph +802，复测零产出依旧，判决表已刷新到最新代码口径）；zod/execa 从 scratch 挪入编制；新克隆 spring-petclinic（Java）/ okhttp（Kotlin）/ cobra（Go）/ cJSON（C）/ fmt（C++）/ vue-realworld（Vue）/ realworld（Svelte）补六种裸奔语言。新建 `reference/README.md` 记编制 + 纪律（无闸仓不可信，先补闸再测；GitNexus 的 JVM 文件是 fixture 勿当基准）。
