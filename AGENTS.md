...
**规模**
- 111 文件，56 主代码 + 0 参考（archive 目录已被 file-index 排除）
- 角色：entry=4, library=44, test=51, script=12
...
- 测试覆盖度极高（51 test vs 44 library），核心改动均有测试兜底
...
| 解析器 | Tree-sitter 原生绑定（14 种语言） | babel/regex（9 种） | **高** — 语言注册表配置模式 |
