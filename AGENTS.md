...
> <!-- generated: 2026-05-08 — 数据由 `node cli.js audit-summary --cwd .` 自分析得出，每次结构大幅变化后需重新运行并更新本段。-->
>
> 供开发者快速建立心理模型。

**规模**
- 159 文件，74 主线 + 85 非主线（test/docs）
- 角色：entry=1, library=60, test=85, script=12, unknown=1
- 入口：`cli.js`（CLI 入口）
...
- 测试覆盖度极高（85 test vs 60 library），核心改动均有测试兜底
...
> 注：`stack-detector.js`（835→14）和 `file-index.js`（523→420）拆分已完成；`dep-graph.js`（~1088 行）保持不物理拆分，已验证内聚优先决策正确。新增 `isLikelyFrameworkLegitimateCycle` 方法（~30 行）和 `isKnownEntryFile` content-based 检测整合，内聚性未受损。
