...
THEN 拿到结果后必须执行：
  ...
  3. 收工前必须跑 `npm run test:fast` 并 120/120 PASS，确认无回归
```
...
*Last updated: 2026-06-16（修复 audit-diff .gitignore 误报；CLI help 与框架检测表驱动化；缓存目录排除与产物过滤；npm run test:fast 120/120 PASS，npm run test:smoke 123/123 PASS，npm run test:coverage:check 通过；schemaVersion: 1.2.0；version: 2.0.0）*
