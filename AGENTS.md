...
| 命令 | 输出 | 适用场景 |
|------|------|----------|
| `workspace-info` | 项目元数据 | 首次探测仓库结构 |
| `diagnostics` | 诊断结果（eslint/tsc/pyright 等）| 检查当前文件类型错误 |
| `impact --file` | 影响半径 | 精确查询单文件影响面 |
| `affected-tests --file` | 测试列表 | 精确查询单文件关联测试 |
| `dead-exports` | 死导出列表 | 清理未使用代码 |
| `unresolved` | 未解析 import 列表 | 修复依赖路径 |
| `cycles` | 循环依赖列表 | 架构重构参考 |
| `stats` | 项目统计 | 快速获取文件/边/循环数 |
| `dependencies --file` | 依赖列表 | 查询文件直接依赖 |
| `dependents --file` | 被依赖列表 | 查询文件直接反向依赖 |
| `health` | 健康检查 | 快速评估 CI/测试/配置完整性 |
| `audit-security` | 安全发现 | 依赖 Semgrep，检测漏洞 |
| `watch` | 持续输出 | 文件保存自动打印影响面 |
...
**规模**
- 117 文件，117 主代码 + 0 参考（archive 目录已被 file-index 排除）
- 角色：entry=4, library=47, test=54, script=12
...
| 层级 | 代表文件 | 职责 |
|------|----------|------|
| L0 基础设施 | `path.js`(17↑), `constants.js`(16↑), `sanitize.js` | 路径工具、常量、脱敏 |
...
|------|-----------|-----------|-----------|------|
| `src/utils/path.js` | 17 | 47 | 27 | 最底层基础设施，任何路径逻辑变动波及面最大 |
| `src/config/constants.js` | 16 | 52 | 29 | 全局常量 |
| `src/services/dep-graph.js` | 10 | 17 | 12 | 导出 `DependencyGraph` 类，核心引擎 |
...
| `src/services/container.js` | 5 | 7 | 4 | ServiceContainer 生命周期管理 |
...
- 测试覆盖度极高（54 test vs 47 library），核心改动均有测试兜底
...
| 解析器 | Tree-sitter 原生绑定（14 种语言） | babel/ast + javalang + web-tree-sitter WASM（9 种，6 种 AST） | **高** — 语言注册表配置模式 |
...
*Last updated: 2026-05-05（深度 audit 后规则更新：新增资源管理铁律、缓存一致性检查、跨文件语义同步、平台兼容性假设、重复代码检测；结构性债务记录在案）*
