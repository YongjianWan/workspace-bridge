...
workspace-bridge 当前解决了**看得准**（compact 模式、热区、影响面）和**改得敢**（audit-file/audit-diff 的验证建议），**错得快**已通过 P8-1 初步落地。

`watch --run-tests` 建立自动闭环：文件保存 → impact → affected-tests → JSON Lines 结果回传。基础设施从"望远镜"进化到"辅助驾驶"——AI 改完代码后，测试自动跑、失败自动报、上下文自动喂。

**下一步：P8-3 增量策展**（`audit-file --watch`、`audit-diff --incremental`），让 AI 只收到与当前改动相关的信息，而非全库噪音。见 [ROADMAP.md §P8](./ROADMAP.md#p8从报告到闭环)。
...
> <!-- generated: 2026-05-09 — 数据由 `node cli.js audit-summary --cwd .` 自分析得出，每次结构大幅变化后需重新运行并更新本段。-->
>
> 供开发者快速建立心理模型。

**规模**
- 179 文件，93 主线 + 86 非主线（test/docs）
- 角色：entry=1, library=61, test=86, script=14, unknown=3
- 入口：`cli.js`（CLI 入口）
...
- 测试覆盖度极高（86 test vs 61 library），核心改动均有测试兜底
...
> 注：`stack-detector.js`（835→14）和 `file-index.js`（523→420）拆分已完成；`dep-graph.js`（~1168 行）保持不物理拆分，内部已拆为 `GraphBuilder` / `GraphAnalyzer` / `GraphQuery` + `DependencyGraph` facade，认知负荷下降。P8-1 变更触发插槽（`onBuildComplete` / `onFileUpdated`）已预留并投入使用。
...
*Last updated: 2026-05-09（路线 A 交付：P85/P70/P71/P79/P80/P81/P72/P73 数据一致性与框架边界硬化；85/85 测试通过）*
