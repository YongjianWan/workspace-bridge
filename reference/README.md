...
# 附录：测量基准仓编制（2026-07-28 满编）

`--depth 1` 克隆，用作 resolver 精度基准（`scripts/resolver-precision.js`）与 dogfood 样本。
不进父仓 git（见本目录 `.gitignore`），可随时 `git pull` 或删了重克。
选择标准：真实项目（非 fixture 合集）、有 manifest、中小型、相对知名、尽量活跃。

| 仓 | 语言 | manifest | 闸状态 |
| --- | --- | --- | --- |
| GitNexus | TS（另含 Java/Kotlin/Rust fixture，**勿当 JVM 基准**） | package.json | ✅ 有闸 |
...
纪律：没有闸的语言 precision 数据不可信（假边混在命中数里），无闸仓先入编、后补闸、再测量；
文档引用的数字对应测量时快照，pull 更新后注意新鲜度；取数逐仓点名，勿用 `reference/*` 通配。
