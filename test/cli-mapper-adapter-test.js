...
async function run(args) {
  return await runCliInProcessRaw([...args, '--cwd', tempDir], { cwd: tempDir });
}
...
async function main() {
  ...
  try {
    ...
    {
      const result = await run(['audit-diff', '--json', '--quiet']);
    ...
    {
      const result = await run(['affected-tests', '--file', 'index.js', '--max-depth', 'abc']);
    ...
    {
      const result = await run(['audit-diff', '--reuse-hints', 'maybe', '--json']);
    ...
    {
      const result = await run(['audit-overview', '--trend-granularity', 'hour', '--json']);
    ...
    // Test 6: Non-existent file for impact/dependents/dependencies should exit 1 in human mode
    {
      ...
      for (const cmd of commands) {
        const result = await run([cmd, '--file', 'nonexistent-file-xyz.js']);
    ...
    {
      const result = await run(['audit-diff', '--token-budget', '-100']);
