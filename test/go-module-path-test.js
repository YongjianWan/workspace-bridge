...
function main() {
  ...
  assert(!singleFocused.cmd.includes('cd '), `Single-module should not cd: ${singleFocused.cmd}`);
  ...
  assert(backendFocused[0].cmd.includes('cd backend && '), `Backend should cd into module: ${backendFocused[0].cmd}`);
  ...
  const backendCmd = multiFocused.find((c) => c.cmd.includes('cd backend'));
  const frontendCmd = multiFocused.find((c) => c.cmd.includes('cd frontend'));
  ...
  assert(!rootFocused[0].cmd.includes('cd '), `Root module should not cd: ${rootFocused[0].cmd}`);
  ...
  const backendTestCmds = directResult.focused.filter((c) => c.cmd && c.cmd.includes('cd backend'));
  assert.strictEqual(backendTestCmds.length, 1, 'Should dedupe identical focused/direct test commands');
  assert(backendTestCmds[0].cmd.includes('cd backend && '), `Direct backend should cd into module: ${backendTestCmds[0].cmd}`);
