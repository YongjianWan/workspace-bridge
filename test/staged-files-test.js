...
async function testStagedFlagParsing() {
  // --staged should be accepted without error
  const result = await run(['audit-diff', '--staged']);
...
async function testFilesFlagAuditDiff() {
  const files = 'src/utils/path.js,src/utils/constants.js';
  const result = await run(['audit-diff', '--files', files]);
...
async function testFilesFlagAuditSecurity() {
  ...
  try {
    ...
    const result = await runCliInProcessRaw(['audit-security', '--cwd', tempDir, '--builtin-only', '--files', tmpFile, '--json', '--quiet'], { cwd: tempDir });
...
async function testStagedAndFilesMutualExclusion() {
  // When --files is provided, --staged should be ignored (files takes precedence)
  const files = 'src/utils/path.js';
  const result = await run(['audit-diff', '--staged', '--files', files]);
...
async function testInvalidFilesNonExistentAuditDiff() {
  // Non-existent files should still be included in the output (graceful handling)
  const result = await run(['audit-diff', '--files', 'nonexistent-file-12345.js']);
...
async function main() {
  await testStagedFlagParsing();
  await testFilesFlagAuditDiff();
  await testFilesFlagAuditSecurity();
  await testStagedAndFilesMutualExclusion();
  await testInvalidFilesNonExistentAuditDiff();
}
