...
| Spring Boot 入口文件 summary 缺失 | ⏳ 待修复 | `audit-summary` / `audit-overview` 的 `entryFiles` 遗漏 `*Application.java` | `project-context.js` 接入 `framework-patterns.js` 检测 |
| Django 配置驱动入口覆盖不全 | ⏳ 待修复 | middleware、database router、context processors、forms、templatetags 未被识别为框架入口 | 扩展 `FRAMEWORK_MANAGED_PATTERNS` + `framework-patterns.js` |
| Java 常量类死导出误报 | ⏳ 待修复 | `HttpStatus`/`Constants`/`UserConstants` 等常量仓库模式被大量误报 | 识别常量仓库模式并降级 confidence |
| Java 循环依赖无白名单 | ⏳ 待修复 | 领域模型/工具类循环被报告，但通常是正常设计 | 参考 Vue store→router→view 白名单添加 Java 模式 |
| React 循环依赖无白名单 | ⏳ 待修复 | `isLikelyFrameworkLegitimateCycle` 仅覆盖 Vue | 添加 React context→hook→component 合法循环过滤 |
| `audit-summary` vs `cycles` 数据不一致 | ⏳ 待修复 | zcypg_frontend 的 audit-summary 报告 7 个 cycles，cycles 命令仅返回 3 个 | 统一 `_cycleCount` 计算路径与过滤逻辑 |
| 脚手架噪音淹没业务信号 | ⏳ 待修复 | 两个 RuoYi 仓库 35+ 同名 dead exports，cycles/health 完全重合 | 引入脚手架指纹检测，将公共代码标记为 `scaffold` |
| Spring/Quartz/MyBatis 组件误报 | ⏳ 待修复 | Filter/Wrapper/Validator/RedisSerializer/TypeHandler/QuartzJob 被标 dead-export | 扩展 `framework-patterns.js` 覆盖运行时装配组件 |
| Maven 项目 `testFiles: 0` | ⏳ 待修复 | 两个 Spring Boot 多模块项目均有 `src/test/java` 但 testFiles=0 | 修复 `isTestLikeFile()` 对 Java 测试命名的覆盖 |
| Windows 路径大小写归一化 | ⏳ 待修复 | `filePreview.js` 输出为 `filepreview.js` | 评估 `normalizePathKey` 的跨平台影响 |
| `audit-summary` orphans 聚合与明细不一致 | ⏳ 待修复 | `ai_gwy_backend` orphans total(4) ≠ 明细加总(2) | 统一 `overview-tools.js` 与 `project-map.js` 的 orphan 计数逻辑 |
...
9. **闭环验证**（文件改动 → 自动计算影响 → 自动跑相关测试 → 结果自动回传 AI）— ✅ P8-1 已完成，`watch --run-tests` 可用；P8-3 增量策展待评估
...
### P8-0：架构前置——dep-graph.js 内部拆分 ✅ 已完成

**问题**：`dep-graph.js` 同时承载 6 项职责（图构建、查询接口、分析算法、框架感知、符号级分析、增量更新），已发展为 God Class（~1121 行，coupling total=25）。P8-1 需要在其中插入"变更触发"逻辑，硬塞会让改动成本指数增长。

**方案**：对外接口 100% 不变，内部拆为三个 collaborator：
- `GraphBuilder` — `build()` / `updateFiles()` / watcher 集成
- `GraphAnalyzer` — `findDeadExports()` / `findCycles()` / `findUnresolved()`
- `GraphQuery` — `getImpactRadius()` / `getAffectedTests()` / `getDependencies()`

`DependencyGraph` 类退化为 facade，委托给三个内部对象。不物理拆文件（AGENTS.md 内聚优先原则不变），但内部认知负荷下降。

**工作量**：0.5 天。**已完成**，验证：85/85 测试通过，healthScore=5/5。

**P8-1 插槽已预留**：`GraphBuilder.onBuildComplete` / `GraphBuilder.onFileUpdated`。

### P8-1：watch 闭环 ✅ 已完成

`watch` 当前只是"文件变了打印影响"的演示。进化目标：

```
文件保存 → 自动计算 impact → 自动跑 affected-tests →
测试结果（PASS/FAIL + 失败详情）回写成 AI 可消费的 JSON/结构化格式 →
AI 在下一轮上下文中自动修正 → 循环
```

**已交付**：
- CLI 新增 `--run-tests` 标志，开启闭环模式
- `watch.js` 内新增 `buildWatchValidationCommands` / `executeWatchCommand` / `runWatchValidation`
- 文件保存后自动调用 `depGraph.findAffectedTests` → `generateCommands`（`run-direct-tests` steps）→ spawn 执行 focused 测试命令
- 输出格式：JSON Lines（`validationStart` / `commandStart` / `commandResult` / `validationComplete`）
- 失败处理：任一命令 exitCode ≠ 0 立即停止，失败详情（stdout/stderr）写入 `commandResult` 事件
- 超时保护：单命令 60s 超时自动 kill
- 验证：85/85 测试通过，`test/watch-test.js` 新增 `--run-tests` 闭环覆盖
...
当前 `validationAdvice.commands` 里的命令是**建议文本**，AI 需要二次解析才能执行。进化目标：

- `commands` 数组从字符串升级为结构化对象：`{ command, args, cwd, whenToRun, expectedExitCode, onFailure }`
- 上层系统（agent 框架）可直接调用，无需 NLP 解析
- `audit-diff` 的 smoke/focused/full phases 可被自动化流水线直接消费
- **无架构阻力**：只改 formatter 层和 `stack-detectors/commands.js`，不动核心引擎

### P8-3：增量策展（P1）

当前 `audit-overview` / `audit-map` 输出全库热区图，AI 改一个文件时，整个热区图是噪音。进化目标：

- `audit-file --watch`：只输出"与当前文件相关的"影响面、历史风险、测试映射
- `audit-diff --incremental`：只输出"本次改动引入的新问题"，而非全量问题
- 预计算洞察有保质期，动态场景下需要按需策展
- **依赖 P8-0**：需要 GraphQuery 的增量查询能力
...
| watch 闭环 | 文件改动后无法自动验证 | ✅ `watch --run-tests` 已可用 | — | — |
| validationAdvice 可执行 | AI 需 NLP 解析命令文本 | ✅ `commands` 新增 `executable` 结构化字段 | — | — |
| 增量策展（P8-3）| AI 改一个文件时收到全库噪音 | ⏳ 完全空白，无代码实现 | `audit-file --watch` / `audit-diff --incremental` 按需策展 | 2–3 天 |
| 脚手架噪音过滤 | 同模板项目审计输出被共性代码淹没 | ⏳ 无实现 | 引入脚手架指纹（RuoYi/Vue Admin 等）标记公共代码 | 1–2 天 |
...
| P1 | `file-index.js:285-288` | `content.split('\n')` 仅数行数 | 1MB 文件 → 临时数组 **~20MB**；并发 50 个时峰值 **~1GB** | ✅ 已修复 v1.1.0：改用 `content.match(/\n/g)?.length + 1` |
| P1 | `dep-graph.js:655` | `_scanLocalSymbolUsage` 仍用 `content.split('\n')` | 同 file-index.js 已修复问题，dead-export 分析时复现内存峰值 | 改用流式扫描或 `match(/\n/g)` |
| P1 | `framework-usage-patterns.js` | `resolveImplicitImports` 无缓存 `fs.existsSync` | 每次 build/updateFiles 重复同步 I/O | 复用 `cachedExistsSync` |
...
| 2 | TS/Python/前端项目可信主线结论 | **80%** | Spring Boot `*Application.java` 在 summary 中缺失；Django middleware/router 误报严重；React 循环无白名单 |
...
| 5 | 大仓库性能可接受 | 95% | 10k+ 文件首次索引未实测；两个 P0 性能瓶颈已修复；`_scanLocalSymbolUsage` 内存峰值待修 |
| 6 | 可选外部工具后端（Semgrep）| 100% | — |
| 7 | 全栈语言覆盖（9 种）| 100% | — |
| 8 | 全栈 AST 覆盖（9/9 语言）| **100%** | — |
| 9 | 闭环验证（P8）| **70%** | P8-1 `watch --run-tests` ✅ 已可用；P8-2 validationAdvice 可执行 ✅ 已完成。剩余：P8-3 增量策展（零实现）、onGitStaged 触发、失败信息注入 AI 上下文 |
...
- 测试覆盖补全（flaky 测试修掉）
- `project-context.js` 与 `framework-patterns.js` 入口识别统一（P70）
- Django 配置驱动入口扩展（P71）
- Java 常量仓库模式误报收敛（P72）
- 跨框架循环白名单公平性（Vue ✅ / Java ⏳ / React ⏳）

### 产品方向诚实评估

**P8-3 增量策展是文档驱动的虚假承诺。**

AGENTS.md 和 ROADMAP.md 均将 P8-3 列为"下一步方向"，但代码中零实现：
- `audit-file --watch` 不存在
- `audit-diff --incremental` 不存在
- `GraphQuery` 无增量查询能力
- 无增量策展的数据结构设计

**这不是技术债，是产品叙事与工程现实的脱节。** 要么在下一轮投入 2–3 天实现 MVP，要么将 P8-3 从"下一步"降级为"远期构想"，停止在文档中制造虚假预期。

**跨框架公平性赤字。**

当前框架支持呈现明显的 "Vue 优先" 特征：
- Vue：循环白名单、隐式依赖注入（router 懒加载 / 全局组件）、框架模式识别、SFC 解析
- React：仅有基础 JS parser 和 jsx 扩展名支持，无 React 特异性循环白名单、无 hooks 隐式依赖
- Java/Spring Boot：有 AST 注解检测和框架模式识别，但无循环白名单、summary 入口缺失
- Django：有 views/admin/management/tasks 识别，但 middleware/router/context processors 大面积遗漏

若要维持"全栈覆盖"的品牌定位，需要在非 Vue 语言上补齐同等水平的框架感知能力。

**脚手架噪音是产品可用性的隐形杀手。**

实战基地中两个 RuoYi 后端和两个 Ruoyi Vue 前端的审计输出高度同质化：
- 后端 dead-exports 重合度 > 90%，cycles 完全重合（3 组），health 一字不差
- 前端 dead-exports 大量重合（`ruoyi.js`、`permission.js`、`validate.js`、`generator/*.js`）

这意味着基于常见脚手架构建的项目，workspace-bridge 的首次审计输出中 **> 80% 是已知共性代码**，真正需要开发者关注的业务代码被淹没在噪音中。当前工具没有"脚手架指纹"或"公共代码折叠"能力，AI 需要自行识别"这是 RuoYi 的已知代码，可以忽略"。

**修复优先级判断**：脚手架噪音（P78）对"工具可用性"的伤害，可能大于 P8-3 增量策展对"工具先进性"的提升。如果 AI 拿到 40 条 dead exports 需要手动过滤 35 条，工具的核心价值就被稀释了。

---

*Last updated: 2026-05-09（两轮实战基地审核：六仓库端到端 + 代码深度审查；新增 P70–P91 共 22 项技术债；P8-3 产品方向诚实评估；框架边界硬化路线确立；85/85 测试通过）*
