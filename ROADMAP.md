...
| ~~**端到端请求路径（路由提取）**~~ ✅     | `framework-patterns.js` / `dep-graph.js` / `human-formatters.js` | **接受 / 已交付**     | 已交付。将路由提取前置到 AST 解析阶段，用 `handles-route` 边表示关系并实现 Graph-first 内存图 BFS 查询，完全取代二次磁盘扫描，显著提升 impact 命令速度。详见 [CHANGELOG.md](./CHANGELOG.md)。 |
...
| ~~**框架检测 query 化（compilePatterns）**~~ ✅ | 中   | 中   | **接受 / 已交付**     | 已交付。JS/TS、Python、Java、Kotlin、Go、Rust、Vue、Svelte 均已完成 AST-Query 化并整合到统一的 `FRAMEWORK_QUERY_REGISTRY`。详见 [CHANGELOG.md](./CHANGELOG.md)。 |
...
| ~~**缓存目录默认化到项目内**~~ ✅          | 高   | 低   | **接受 / 已交付**     | 已交付。默认缓存路径已从系统的临时目录迁移至本地项目的 `.workspace-bridge/` 中，且处理好 `.gitignore` 以防泄漏。详见 [CHANGELOG.md](./CHANGELOG.md)。 |
| ~~**用户级配置目录**~~ ✅                 | 中   | 低   | **接受 / 已交付**     | 已交付。支持 `~/.workspace-bridge/config.toml` 与 `.env` 配置文件，并建立完善的配置链与来源报告。详见 [CHANGELOG.md](./CHANGELOG.md)。 |
| ~~**跨进程并发控制**~~ ✅                 | 中   | 低   | **接受 / 已交付**     | 已交付。通过 PID 锁文件控制跨进程 SQLite 写入事务冲突，且具备对损坏/空锁文件的自动解锁和退避重试能力。详见 [CHANGELOG.md](./CHANGELOG.md)。 |
