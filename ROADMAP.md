...
| mixed repo 技术栈启发式         | 🔄 L1/L2 已交付，L3 规划中 | 无归属文件（Dockerfile/CI/.env）变更无命令；多栈同时变更缺少跨栈兜底建议 | L1: 无归属文件触发全栈 smoke 提醒（`mixed-infra-smoke`）；L2: 2+ 栈变更追加 `cross-stack-full-tests` 兜底；L3: 框架感知跨栈依赖映射（Django+React 等组合）待评估 |
...
| audit-security Rule ID 映射错位 | ✅ 已修复 | Markdown 使用 `ruleId`，JSON 中 `rule` 可能为 null（外部 scanner 只提供 ruleId） | `src/tools/security-tools.js` 统一补齐 `rule = rule || ruleId`；`test/security-adapter-test.js` + `test/security-ruleId-test.js` 防止回归 |
...
| Vue kebab-case 模板标签不成边 | ⏳ 已文档化 | 模板中 `<the-header/>` 对应 `TheHeader` import 的 kebab-case 写法抽不出边（只匹配 PascalCase tag 与本地绑定同名命中） | 实测 `reference/vue-realworld-example-app` 全仓 kebab 仅 `router-link`/`router-view`（全局注册，本不该造边）；组件请用 PascalCase 模板写法 |
...
| # | 目标                                     | 改动文件                            | 预期收益                                                        | 工作量 | 状态      |
| - | ---------------------------------------- | ----------------------------------- | --------------------------------------------------------------- | ------ | --------- |
| 1 | **SKILL.md 按层级重组**            | `skills/workspace-audit/SKILL.md` | 从 333 行缩至 ~112 行；只保留默认参数、核心决策树、何时不用、预热工作流、安全清单 | ~30 行 | ✅ 已完成 |
...
| 进度     | ✅ 已完成 | 超大仓库（>10k 文件）索引进度粒度不足；用户不知道是在工作还是卡住了 | `ServiceContainer._runStage` 输出阶段进度；`FileIndex` 按百分比输出索引进度（`[FileIndex] 24% (100/419 files indexed)`） |
