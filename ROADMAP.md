...
| Wave 11-15 功能多语言等价性偏斜 | ⏳ 架构债务（第一轮已补齐） | AST Rules 字段已覆盖 Python/Go/Rust/C/C++；Shadow Candidates 已覆盖 Python/C/C++；Java branchCount/maxArms 已顶层化。剩余：跨语言 AST 规则内置规则、Go/Rust/Kotlin/C/C++ branchCount/maxArms、框架路由 Query、Vue/Svelte shadow | 详见 [docs/TECH_DEBT.md](./docs/TECH_DEBT.md) §Wave 11-15 多语言功能等价性债务；按语言优先级逐步补齐 |
