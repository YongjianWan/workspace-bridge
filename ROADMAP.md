...
| ~~**测试间隙穿透（Dispatcher Regex）**~~ ✅ | `affected-tests` 逻辑扩展                                            | 已交付（Wave 10-3）。无 import 边但测试文件 body 提及源文件 stem，或 `call_tool_by_name("xxx")` 字符串分发匹配到的测试，也纳入 affected-tests。详见 [CHANGELOG.md](./CHANGELOG.md) [Unreleased]。 | 需避免与已有 import 边重复计数           |
| ~~**Bus Factor / 知识分布**~~ ✅          | `overview-tools.js`                                                  | 已交付。`audit-overview` 新增 `knowledgeRisk` 维度：逐文件 `git blame` + mailmap 去重，标识"只有一个人懂的文件"。详见 [CHANGELOG.md](./CHANGELOG.md) [Unreleased]。 | 依赖 git 历史完整，新仓库无意义          |
| ~~**回归测试档案**~~ ✅                   | `test/` 新增 `fp_regression_*.js`                                  | 已交付（Wave 9-3）。死代码/安全/未解析 import 的已知误报场景归档，防止修复后复发。详见 [CHANGELOG.md](./CHANGELOG.md) [Unreleased]。 | 档案需随规则调整同步维护                 |
...
| ~~**测试间隙穿透（Dispatcher Regex）**~~ ✅ | 中   | 低   | **接受 / 已交付**     | 已交付（Wave 10-3）。`affected-tests` 引入 qartez Dispatcher Regex + FTS Stem Mention 回退：无 import 边但测试文件 body 提及源文件 stem 时也纳入。详见 [CHANGELOG.md](./CHANGELOG.md) [Unreleased]。                                                                                                                                                     |
| ~~**Bus Factor / 知识分布**~~ ✅          | 中   | 低   | **接受 / 已交付**     | 已交付。`audit-overview` 新增 `knowledgeRisk`：逐文件 `git blame` + mailmap 去重，标识"只有一个人懂的文件"。详见 [CHANGELOG.md](./CHANGELOG.md) [Unreleased]。                                                                                                                                                                                                                                               |
| ~~**回归测试档案（fp_regression_*.js）**~~ ✅ | 中   | 低   | **接受 / 已交付**     | 已交付（Wave 9-3）。死代码/安全/未解析 import 的已知误报场景归档，防止修复后复发。详见 [CHANGELOG.md](./CHANGELOG.md) [Unreleased]。                                                                                                                                                                                   |
| ~~**路径参数安全清洗**~~ ✅               | 中   | 低   | **接受 / 已交付**     | 已交付。`--file`/`--cwd` 等路径参数在进入 graph 前统一清洗，拒绝 `../` 逃逸。参考 qartez `_sanitize_name` Prompt 注入防御。**追加**：CGC 因 Windows 反斜杠导致 `STARTS_WITH` 查询静默失败的教训（`Path.resolve()` → `as_posix()` 强制正斜杠）。workspace-bridge `path.js` 已审计并修复跨平台路径回归。详见 [CHANGELOG.md](./CHANGELOG.md) [Unreleased]。                                                                                                                                                                                                                                                                    |
| ~~**自适应架构边界（`audit-boundaries`）~~ ✅ | 中高 | 中   | **接受 / 已交付**     | 已交付（Wave 11-1）。读取 `.workspace-bridge.json` 可选 `boundaries[]` 字段，用 minimatch 遍历 edges 做违规检测；无配置时用目录层级聚类自动生成建议规则。参考 qartez `BoundaryRule` + CRG Leiden 聚类（edge weight 调参 + cohesion 计算 + 跨社区耦合 >10 条边警告）。详见 [CHANGELOG.md](./CHANGELOG.md) [Unreleased]。                                                                                                                                                                                               |
| ~~**符号解析置信飞轮**~~ ✅               | 中高 | 中   | **接受 / 已交付**     | 已交付（Wave 10-2）。Pre-scan 粗定位（全局 `symbol → [file]` 映射）→ Query 精确捕获 → CRG Tier 置信标注。输出每条边附 `confidence` + `tier` + `resolutionMethod`。详见 [CHANGELOG.md](./CHANGELOG.md) [Unreleased]。                                                                                                                                                                     |
| ~~**增量更新终极协议（四层叠加）**~~ ✅   | 高   | 中   | **接受 / 已交付**     | 已交付（Wave 15-4）。L1 git diff → L2 SHA-256 过滤 → L3 Neighbor-aware 重解析 → L4 WAL Cadence。按层渐进。详见 [CHANGELOG.md](./CHANGELOG.md) [Unreleased]。                                                                                                                                                                                                      |
| ~~**复杂度趋势分析**~~ ✅                 | 中   | 中   | **接受 / 已交付**     | 已交付（Wave 11-3）。`git revwalk` 遍历 commit，tree-sitter 重新解析记录各符号 CC 和行数，输出 `GROWING/SHRINKING/STABLE`。详见 [CHANGELOG.md](./CHANGELOG.md) [Unreleased]。                                                                                                                                                                                                                   |
| ~~**代码异味检测（Flat Dispatcher）**~~ ✅ | 中   | 中   | **接受 / 已交付**     | 已交付（Wave 11-2）。扫描 `switch(action.type)` / `if-elif` 链，识别平铺 match（`arms ≥ 6` 且 `cc ≤ arms + 5`）和 Dominant 分支（`arms ≥ 12` 且 `arms ≥ cc × 0.4`）。详见 [CHANGELOG.md](./CHANGELOG.md) [Unreleased]。                                                                                                                                                               |
| **SQLite pragma 调优**                     | 中   | 极低 | **接受 / 已交付**     | WAL + mmap + temp_store 调优，提升缓存写入和查询性能。P0 已交付。参考 qartez SQLite 配置                                                                                                                                                                                                                                                                                 |
| ~~**Pre-scan 全局符号映射**~~ ✅          | 中高 | 中   | **接受 / 已交付**     | 已交付（Wave 10-1）。正式解析前轻量 query 提取所有文件顶层定义名，构建 `imports_map = {symbol_name: [file_path]}`。详见 [CHANGELOG.md](./CHANGELOG.md) [Unreleased]。                                                                                                                                                  |
| ~~**规则引擎层次 A（配置化）**~~ ✅       | 中   | 低   | **接受 / 已交付**     | 已交付（Wave 14-1）。将 `security-tools.js` 硬编码规则提取为外部 YAML/JSON，无需数据库。通过 `--config <file>` 参数接入，不新增 `rules` 子命令。详见 [CHANGELOG.md](./CHANGELOG.md) [Unreleased]。                                                                                                                                                     |
| ~~**规则引擎层次 B（AST 轻量规则）**~~ ✅ | 中高 | 中   | **接受 / 已交付**     | 已交付（Wave 15-1）。基于现有 `functionRecords` 做方法级条件检查（如"batch* 方法无 @Transactional"），不跨文件。详见 [CHANGELOG.md](./CHANGELOG.md) [Unreleased]。                                                                                                                                                                |
...
| ~~**AI 摘要输出（纯模板）**~~ ✅          | 高   | 低   | **接受 / 已交付**     | 已交付。`--format summary` / `--format markdown` 用模板将 JSON 策展为 20 行关键结论或 Markdown 审查意见，不引入 LLM 调用。详见 [CHANGELOG.md](./CHANGELOG.md) [Unreleased]。                                                                                                                                                                                                                                                       |
| ~~**增量分析扩展**~~ ✅                   | 高   | 低   | **接受 / 已交付**     | 已交付。`--since <commit>` commit range、`--staged` 暂存区、`--files a,b,c` 指定文件列表、`--with-impact` 变更+依赖方自动展开。详见 [CHANGELOG.md](./CHANGELOG.md) [Unreleased]。                                                                                                           |
| ~~**跨调用缓存（ParseCache）**~~ ✅        | 高   | 中   | **接受 / 已交付**     | 已交付（Wave 15-3）。引入按 `mtime_ns` 失效的 AST + ident + calls 内存缓存，优化"先 impact 再 affected-tests 再 audit-summary"连续查询场景。详见 [CHANGELOG.md](./CHANGELOG.md) [Unreleased]。                                                                                                                                                                                       |
| ~~**统一 risk scoring（5 维度）**~~ ✅     | 高   | 低   | **接受 / 已交付**     | 已交付（Wave 11-4）。`audit-diff` 引入跨文件变更风险排序：flow_participation + community_crossing + test_coverage + caller_count + security_sensitive。详见 [CHANGELOG.md](./CHANGELOG.md) [Unreleased]。                                                                                                                                                                                   |
| ~~**诚实截断机制（Honest Truncation）**~~ ✅ | 中   | 极低 | **接受 / 已交付**     | 已交付（Wave 12-1）。`impact` / `affected-tests` 在大型单体项目中输出可能爆炸时，给结果数组包装 `truncated` 布尔字段，超限后诚实告知"已截断，仅展示前 N 个"。详见 [CHANGELOG.md](./CHANGELOG.md) [Unreleased]。                                                                                                                                                                                         |
...
| ~~**环境变量层 + 配置来源报告**~~ ✅     | 低   | 低   | **接受 / 已交付**     | 已交付（Wave 14-3）。增加 `WB_*` 环境变量层和启动时"来源报告"（config from: env > cli > file）。详见 [CHANGELOG.md](./CHANGELOG.md) [Unreleased]。                                                                                                                                                                                        |
| ~~**JSON 输出 token 削减**~~ ✅           | 中   | 低   | **接受 / 已交付**     | 已交付（Wave 12-2）。大型文件 `--json` 输出前通过 `elideDeep()` 自动截断超大数组/字符串，削减 token 体积。详见 [CHANGELOG.md](./CHANGELOG.md) [Unreleased]。                                                                                                                                                                                                         |
...
| ~~**GraphBuilder / GraphAnalyzer 职责边界**~~ ✅ | 中   | 低   | **接受 / 已交付**     | 已交付（Wave 13-2）。对外接口显式区分"节点构建期"和"边链接期"，文档化 + 生命周期 hook。详见 [CHANGELOG.md](./CHANGELOG.md) [Unreleased]。                                                                                                                                                                                                                        |
| ~~**语言注册表统一契约**~~ ✅              | 中   | 低   | **接受 / 已交付**     | 已交付（Wave 13-1）。引入 `{ language, extensions, parse, extractImports, extractExports, isBuiltIn }` 配置表，统一 parserAvailability / import 解析 / 导出检测。参考 GitNexus `satisfies Record<SupportedLanguages, LanguageProvider>` 编译时穷举表 + `LanguageProvider`/`ScopeResolver` 双契约拆分。详见 [CHANGELOG.md](./CHANGELOG.md) [Unreleased]。                                                                                                                                                         |
| **框架检测 query 化（compilePatterns）**   | 中   | 中   | **接受 / 部分已交付** | JS/TS (Express)、Python (Django/FastAPI/Flask/Celery)、Java (Spring/Spring Boot)、Kotlin (Spring/Ktor) 已完成 AST-Query 化；Go/Rust/C/C++/Vue/Svelte 仍依赖 regex/cheap-signature。下一步继续补齐剩余语言并收敛到统一 `FRAMEWORK_QUERY_REGISTRY`。参考 GitNexus `HttpLanguagePlugin` 契约 + graph-first 路由提取策略。详见 [CHANGELOG.md](./CHANGELOG.md) [Unreleased] 与 [docs/TECH_DEBT.md](./docs/TECH_DEBT.md)。                                                                                          |
...
| ~~**分层输出过滤**~~ ✅                   | 中   | 低   | **接受 / 已交付**     | 已交付（Wave 12-3）。`--severity high|medium|low` 按严重程度过滤、`--category dead-exports/unresolved/cycles/health` 按类别过滤。详见 [CHANGELOG.md](./CHANGELOG.md) [Unreleased]。                                                                                                                                                                                                         |
| ~~**审查追踪（轻量）**~~ ✅               | 中   | 低   | **接受 / 已交付**     | 已交付。`--save <file>` 保存审计结果、`--check-regression` 对比上次审计检查 P0/P1 是否修复、`--baseline <commit>` 按变更文件标注问题为 `new`/`legacy`。详见 [CHANGELOG.md](./CHANGELOG.md) [Unreleased]。                                                                                                                                                                                                                   |
...
| ~~**大项目自动截断/自适应**~~ ✅          | 中   | 低   | **接受 / 已交付**     | 已交付（Wave 12-4）。基于项目总文件数（默认 500，`LARGE_PROJECT_FILE_THRESHOLD`）自动启用 `--compact`，或自动抑制低价值字段；`--no-compact` / `--compact` / `WB_COMPACT` 显式覆盖。详见 [CHANGELOG.md](./CHANGELOG.md) [Unreleased]。                                                                                                                                                                                                         |
| ~~**噪音抑制增强**~~ ✅                   | 中   | 低   | **接受 / 已交付**     | 已交付（Wave 14-2）。`.workspace-bridge.json` 扩展 `ignore` 配置（框架感知排除）、`--mark-false-positive <id>` 记录误报（轻量，不引入机器学习）。详见 [CHANGELOG.md](./CHANGELOG.md) [Unreleased]。                                                                                                                 |
...
| ~~**大项目截断（手动）**~~ ✅             | 低   | 低   | **接受 / 已交付**     | 已交付（Wave 12-5）。`--max-files <n>` 限制 `audit-diff` 变更文件数，并在 `options` 中报告截断状态。详见 [CHANGELOG.md](./CHANGELOG.md) [Unreleased]。                                                                                                          |
| **缓存目录默认化到项目内**                 | 高   | 低   | **接受**              | 将默认缓存目录从 `os.tmpdir()` 迁移到 `<workspaceRoot>/.workspace-bridge/`，实现跨会话持久化、CI 缓存可命中、legacy 自动迁移。参考 code-review-graph `<repo>/.code-review-graph/` 与 qartez `<project>/.qartez/index.db`。详见 [docs/TECH_DEBT.md](./docs/TECH_DEBT.md)。                                                                                                                                                                                                         |
| **用户级配置目录**                         | 中   | 低   | **接受**              | 引入 `~/.workspace-bridge/` 全局配置，支持多 repo 默认行为统一管理。配置优先级：CLI args > env > 项目级 `.workspace-bridge.json` > 用户级配置 > 默认值。参考 code-review-graph `~/.code-review-graph/watch.toml`。详见 [docs/TECH_DEBT.md](./docs/TECH_DEBT.md)。                                                                                                                                                                                        |
| **跨进程并发控制**                         | 中   | 低   | **接受**              | 在 SQLite 写入路径增加轻量 advisory lock，防护 watch 进程与 CLI 命令之间的写事务竞态。参考 qartez `RepoLock`（OS-level advisory lock + 指数退避 + PID 诊断）。详见 [docs/TECH_DEBT.md](./docs/TECH_DEBT.md)。                                                                                                                                                                                                                                 |
