...
| Wave 11-15 功能多语言等价性偏斜 | ⏳ 架构债务（字段与 shadow 已补齐） | `functionRecords` 字段（decorators/isExported/returnType）与 `branchCount`/`maxArms` 已覆盖 9 种语言；Shadow Candidates 已覆盖 JS/TS/Python/C/C++/Vue/Svelte。剩余：跨语言 AST 规则内置规则、框架路由 Query（FastAPI/Gin/Axum/Nuxt/SvelteKit） | 详见 [docs/TECH_DEBT.md](./docs/TECH_DEBT.md) §Wave 11-15 多语言功能等价性债务；按语言优先级逐步补齐 |
